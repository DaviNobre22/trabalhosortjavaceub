...
public class Shell {
	...
	public static void ordenarLista() {
		...
		/* n é o tamanho da lista e o h vai definir o pulo */
		...
		/*pegando h como numero inteiro e a função math floor para arredondar para baixo */
		h = (int) Math.floor(h / 3);
		int elemento,j;
		
		/*c vai marcar o elemento do vetor que qeuro trocar */
		/*j vai passar pelas posiçoes que são puladas no shell sort */
		while(h > 0){
			for(int i = h; i < n; i ++){
				elemento = listaOrdenada[i];
				j = i;
				/*j começa no valor de  i e vai decrementando */
				while (j >= h && listaOrdenada[j - h] > elemento) {
					listaOrdenada[j] = listaOrdenada[j - h];
					j = j - h;
				}
				listaOrdenada[j] = elemento;
